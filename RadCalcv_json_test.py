...
def json_report(finalreport, bikename):
    finalreport['Bike']=bikename
    nested_report= {}
    nested_report[bikename] = finalreport
    j = json.dumps(nested_report, indent=4)
    with open('json_bikereport.json', 'a') as f:
        f.write(j)
        f.close()
    return j
